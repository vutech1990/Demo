...
<div class="max-w-4xl mx-auto px-4 md:px-0">
    ...
    <div class="mb-8 flex justify-between items-center">
        <a href="/" class="group flex items-center text-sm font-bold text-gray-400 hover:text-blue-600 transition">
            <div
                class="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center mr-2 group-hover:bg-blue-50 transition">
        ...
        <div class="flex space-x-2">
            ...
            <a href="/posts/{{ $post->id }}/edit"
                class="inline-flex items-center px-4 py-2 bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white text-xs font-bold rounded-xl transition shadow-sm">
            ...
            <form action="/posts/{{ $post->id }}" method="POST"
                ...
                @method('DELETE')
                <button type="submit"
                    class="inline-flex items-center px-4 py-2 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white text-xs font-bold rounded-xl transition shadow-sm">
                    <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    {{-- Nội dung bài viết --}}
    <article class="bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/[0.03] border border-gray-100 overflow-hidden">
        {{-- Hero Image --}}
        @if($post->thumbnail)
        ...
        <div class="p-6 md:p-12">
            <header class="mb-10 text-center md:text-left">
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-2 mb-6">
                    ...
                    <span
                        class="px-4 py-1.5 rounded-full bg-{{ $tag->color }}-50 text-{{ $tag->color }}-600 text-[10px] font-extrabold uppercase tracking-widest border border-{{ $tag->color }}-100">
                        {{ $tag->name }}
                    </span>
                    @endforeach
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center">
                        <div
                            class="w-12 h-12 rounded-2xl bg-blue-600 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-blue-200 overflow-hidden">
    ...
    <div id="comment-section"
        class="mt-12 bg-white rounded-[2.5rem] p-6 md:p-12 shadow-xl shadow-blue-900/[0.03] border border-gray-100">
        ...
        {{-- Form Gửi Bình luận --}}
        <div class="bg-gray-50 p-6 md:p-8 rounded-[2rem] mb-12 border border-blue-50/50">
            <h4 class="text-lg font-bold mb-6 text-gray-800">Chia sẻ suy nghĩ của bạn</h4>

            @if(session('success'))
            <div
                class="bg-emerald-50 text-emerald-700 p-4 mb-6 rounded-xl border border-emerald-100 text-sm font-bold flex items-center">
            ...
            <form action="{{ route('comments.store', $post->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="email" name="email" value="{{ Auth::user()->email }}" readonly
                            class="w-full px-5 py-3.5 bg-white border-white rounded-2xl focus:outline-none text-gray-400 font-bold shadow-sm cursor-not-allowed">
                    </div>
                    @else
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name') }}"
                            class="w-full px-5 py-3.5 bg-white border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:outline-none focus:border-transparent transition shadow-sm font-bold placeholder-gray-300">
                    </div>
                    <div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" rows="4" required
                        class="w-full px-5 py-4 bg-white border border-gray-100 rounded-[1.5rem] focus:ring-2 focus:ring-blue-500 focus:outline-none focus:border-transparent transition shadow-sm font-medium placeholder-gray-300"
                        placeholder="Tham gia thảo luận..."></textarea>
                </div>

                <button type="submit"
                    class="w-full md:w-auto px-10 py-4 bg-blue-600 hover:bg-black text-white font-extrabold rounded-2xl transition shadow-lg shadow-blue-100 transform active:scale-95">
                    Gửi bình luận ngay
                </button>
            </form>
        </div>
        ...
        <div class="space-y-8">
            ...
            <div id="comment-{{ $comment->id }}"
                class="comment-block group p-6 md:p-8 rounded-[2rem] bg-white border border-gray-100 hover:border-blue-100 transition-all duration-500">
                <div class="flex justify-between items-start mb-6">
                    <div class="flex items-center">
                        <div
                            class="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white font-extrabold text-xl shadow-md">
                ...
                <div id="reply-form-{{ $comment->id }}" class="hidden mt-8 pl-6 md:pl-12 border-l-4 border-blue-50">
                    <form action="{{ route('comments.store', $post->id) }}" method="POST"
                        ...
                        <textarea name="content" rows="3" required
                            ...
                            class="w-full px-4 py-3 bg-white border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-medium mb-4 transition"></textarea>
                        <div class="flex justify-end space-x-2">
                            <button type="button" onclick="toggleReplyForm({{ $comment->id }})"
                                class="px-5 py-2.5 text-xs font-bold text-gray-400 hover:text-gray-600 transition">Hủy</button>
                            <button type="submit"
                                class="px-6 py-2.5 bg-blue-600 hover:bg-black text-white text-xs font-extrabold rounded-xl transition shadow-md shadow-blue-50">Gửi
                                phản hồi</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="mt-10 space-y-6">
                    ...
                    <div id="comment-{{ $reply->id }}"
                        class="reply-block group/reply bg-gray-50 p-6 rounded-3xl border border-blue-50/30 ml-4 md:ml-12 hover:bg-white hover:shadow-lg hover:shadow-blue-900/[0.02] transition-all duration-300">
                        <div class="flex items-center mb-4">
                            <div
                                class="w-10 h-10 bg-white shadow-sm rounded-xl flex items-center justify-center text-blue-600 font-extrabold text-sm border border-gray-100">
            ...
            @empty
            <div class="text-center py-20 bg-gray-50/50 rounded-[3rem] border border-dashed border-gray-200">
